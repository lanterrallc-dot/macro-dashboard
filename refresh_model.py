...
def usd_transmission(S, window=60):
    """Is the classic "higher yields -> stronger dollar" link actually working?

    Correlation of DAILY CHANGES in the real 10-year yield and the broad dollar
    index — not of their levels, since two trending series can correlate at 0.9
    while saying nothing about transmission.

    The sign is the whole point. When yields rise because growth or the Fed is
    firmer, capital gets pulled in and the dollar rises with them: positive r,
    the textbook channel. When yields rise because investors demand a bigger
    premium to hold the paper at all, that same premium hits the currency and
    the dollar falls as yields rise: negative r. Every USD-routed rationale in
    ASSET_TABLE assumes the first case. This measures which one is actually in
    force, so the dashboard can stop asserting those calls when it is not.
    """
    dfii = S.get('DFII10') or []
    dxy = S.get('DTWEXBGS') or []
    if not dfii or not dxy:
        return None

    dxy_by_date = {p['date']: p['value'] for p in dxy}
    paired = [(p['date'], p['value'], dxy_by_date[p['date']])
              for p in dfii if p['date'] in dxy_by_date]
    if len(paired) < 40:
        return None
    ...
    def corr_over(n):
        ...
        if len(rows) < 30:
            return None, 0
        dy = [rows[i][1] - rows[i - 1][1] for i in range(1, len(rows))]
        dd = [rows[i][2] - rows[i - 1][2] for i in range(1, len(rows))]
        k = len(dy)
        my, md = sum(dy) / k, sum(dd) / k
        cov = sum((dy[i] - my) * (dd[i] - md) for i in range(k))
        vy = sum((v - my) ** 2 for v in dy)
        vd = sum((v - md) ** 2 for v in dd)
        if vy <= 0 or vd <= 0:
            return None, k
        return cov / ((vy ** 0.5) * (vd ** 0.5)), k

    r60, n60 = corr_over(window)
    r120, _ = corr_over(120)
    if r60 is None:
        return None
    ...
    return {
        'r_60d': round(r60, 3),
        'r_120d': round(r120, 3) if r120 is not None else None,
        'n': n60,
        'state': state,
        'label': label,
...
def compute_model(S, E):
    ...
    return {
        ...
        'fx_stress': fxStress,
        'usd_transmission': usd_transmission(S),
        'regime': regime,
...
def main():
    ...
    ut = model.get('usd_transmission')
    print(f"USD transmission: {ut['state']} (60d r={ut['r_60d']})" if ut
          else "USD transmission: not computed")
